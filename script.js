...
function showScreen(id){
  ...
  if(id === 'screen-final') playFinalDriveVideo();
}
// ---------- final surprise (Google Drive video) ----------
// IMPORTANT: the file must be shared (Anyone with the link can view) for embed to work.
const FINAL_DRIVE_VIDEO_ID = '1hsemzdOnoZYZ-wtXKd_STWluaf8p06Qp';
function playFinalDriveVideo(){
  const frame = document.getElementById('finalVideoFrame');
  if(!frame) return;
  // /preview is the embeddable player for Drive files
  const url = `https://drive.google.com/file/d/${FINAL_DRIVE_VIDEO_ID}/preview`;
  // Reload the iframe when entering the screen (helps start playing immediately)
  frame.src = '';
  frame.src = url;
}
...
// ---------- Cursor heart trail ----------
(() => {
  // Optional: disable on touch-only devices (keeps mobile clean)
  ...
  function spawnCursorHeart(x, y){
    ...
    const dy = (-(Math.random() * 26 + 18)).toFixed(1);
  ...
  window.addEventListener('pointermove', (e) => {
    // throttle (about ~30 fps)
    const now = performance.now();
    if (now - last < 32) return;
    last = now;

    // avoid firing while dragging on some browsers
    if (e.buttons) return;
...
// ---------- Intro Mask (heart envelope gate) ----------
function initEnvelopeMask(){
  const mask = document.getElementById('introMask');
  const btn = document.getElementById('openEnvelope');
  if(!mask || !btn) return;

  const open = async () => {
    // Start / unmute music only after envelope click (gesture)
    await unlockMusicWithGesture?.();
    burstHearts?.(16);

    mask.classList.add('hide');
    // remove after transition so it doesn't block clicks
    setTimeout(() => mask.remove(), 620);
  };
